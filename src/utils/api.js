...
export async function geocodeSearch(query) {
  ...
  try {
    const r = await fetch(
      `${NOMINATIM}/search?q=${encodeURIComponent(query)}&format=json&limit=8&addressdetails=1&countrycodes=gb`,
      { headers: UA }
    )
    ...
    return data.map(d => {
      ...
      // For stations show name + town
      const isStation = d.type === 'station' || d.class === 'railway' || parts[0].toLowerCase().includes('station')
...
export async function computeHomeRoutes(from, to, scanResults) {
  ...
  // Transit
  console.log("transitR:", transitR.status, JSON.stringify(transitR.value).slice(0,500)); const plan = transitR.status === 'fulfilled' ? transitR.value?.plan : null
